...
inline XWiFi::XWiFi(const char *iSsid, const char *iPassphrase)
    : mSsid(iSsid), mPassphrase(iPassphrase) {}
...
inline void XWiFi::getStatus(wl_status_t &oStatus) { oStatus = mStatus; }

inline String XWiFi::getMac() {
  byte mac[6];
  WiFi.macAddress(mac);
  char macBuffer[13];
  sprintf(macBuffer, "%02X%02X%02X%02X%02X%02X", mac[0], mac[1], mac[2], mac[3],
          mac[4], mac[5]);
