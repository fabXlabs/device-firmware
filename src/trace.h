...
enum class DebugLevel { INFO_LEVEL, DEBUG_LEVEL, ERROR_LEVEL };

#ifdef DEBUG
#define X_INFO(iMessage, ...)                                                  \
  Trace::trace_message(DebugLevel::INFO_LEVEL, iMessage, ##__VA_ARGS__)
#define X_DEBUG(iMessage, ...)                                                 \
  Trace::trace_message(DebugLevel::DEBUG_LEVEL, iMessage, ##__VA_ARGS__)
#define X_ERROR(iMessage, ...)                                                 \
  Trace::trace_message(DebugLevel::ERROR_LEVEL, iMessage, ##__VA_ARGS__)
#else
#define X_INFO(...)
#define X_DEBUG(...)
#define X_ERROR(...)
...
inline void SerialLogger::begin() { Serial.begin(115200); }

inline void SerialLogger::log(const char *iMessage, DebugLevel iLevel,
                              size_t length) {
  String tag;
