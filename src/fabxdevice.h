...
inline void FabXDevice::loop()
{
    ...
    switch (mCurrentState){
        ...
        case States::IDLE:
        {
            ...
            if(backendState == BackendStates::ERROR) ESP.restart();
        ...
        case States::REQUEST_AUTH_TOOLS:
        {
            ...
            int timeout = 500;
            while(mBackend->getState() != BackendStates::IDLE && timeout > 0) //wait for backend to receive response from server, is list of authorized tools
            {
                wl_status_t oStatus;
                mWifi->loop();
        ...
        case States::TOOL_SELECT:
            ...
            //wait for ValidSecondFactorResponse or ErrorResponse
            {
                ...
                while(!M5.BtnB.wasPressed() && mAuthorizedToolIds.length > 1)
                {
                    ...
                    mDisplay->drawWifiStatus(mCurrentWifiState);
                    mDisplay->drawControls(((mAuthorizedToolIds.length>5)&&(mSelectedTool>4)),((mAuthorizedToolIds.length>5)&&(mSelectedTool<mAuthorizedToolIds.length-1)));
                    mDisplay->drawToolList(mBackend->mTools, mAuthorizedToolIds, mSelectedTool);
                    mDisplay->pushCanvas();
                    delay(10);
                    if(M5.BtnC.wasPressed())
                    {
                        if(mSelectedTool == 0) mSelectedTool = mAuthorizedToolIds.length-1;
                        else mSelectedTool -= 1;
                        now = millis();
                    }
                    if(M5.BtnA.wasPressed())
                    {
                        if(mSelectedTool == mAuthorizedToolIds.length-1) mSelectedTool = 0;
                        else mSelectedTool += 1;
                        now = millis();
                    }
                    if(millis()> now + 10000)
                ...
                for (ITool* tool : mBackend->mTools)
                {
                    if (tool->mToolId == mAuthorizedToolIds.ToolIds[mSelectedTool])
                    {
                        mCurrentTool = tool;
                        X_DEBUG("Tool ID: %s", mCurrentTool->mToolId.c_str());
                        if (tool->mRequires2FA) mCurrentState = States::REQUEST_SECOND_FACTOR;
                        else mCurrentState = States::TOOL_UNLOCK;
        ...
        case States::TOOL_UNLOCK:
        {
            ...
            mBackend->sendToolUnlockedNotification(mCurrentTool->mToolId, mUid, mCardSecret);
        ...
        case States::TOOL_KEEP:
        {
            ...
            while(millis() < (lastValid + mCurrentTool->mTime)){  // wait for time
                 ...
                 if(result == Result::OK) return; //check if card is present, if go back to checking regularly
                 mWifi->loop();
                 mWifi->getStatus(mCurrentWifiState);
                 mBackend->loop(mCurrentWifiState, mCurrentState, mWebsocketState);
                 int secsLeft = (mCurrentTool->mTime) / 1000 - (millis() - lastValid) / 1000;
...
inline void FabXDevice::playRequestSound()
{
    ...
    xTaskCreate(
      playRecSoundTask, /* Function to implement the task */
...
inline Result FabXDevice::addOutputExpander(Adafruit_MCP23008& iOutputExpander)
{
